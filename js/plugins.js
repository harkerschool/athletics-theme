    ...
    Plugin.prototype = {
        init: function () {
            ...
            $("<option />", {
                "selected": "selected",
                "value"   : document.URL,
                "text"    : this.settings.defaultOptionText
            }).appendTo( $select );
            ...
            // Add behavior
            $select.change( this.changeHandler( this.element, this.settings ) );

            // add selection menu
            $select.appendTo( $nav );
            $('html').addClass('selectnav');
            
        },
        changeHandler: function( element, settings ) {
            if ( settings.ajax ) {
                var $container = $( settings.ajaxContainer );
                ...
                return function() {
                    ...
                    settings.beforeLoad();
                    $container.load( location + " " + settings.ajaxSelector, function( response, status, xhr ) {
                        if ( status === "error" ) {
                            window.location = location + "&selectnaverror=true";
                        }
                        settings.success();
                    });
                };
